        ...
        <div className="flex gap-4 mb-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-1 text-sm font-bold text-gray-900">
              <Star size={16} className="fill-[#0056b3] text-[#0056b3]" /> {tutor.rating}
            </div>
          </div>
        </div>
        <div className="flex gap-2 flex-wrap mb-6">
          {tutor.subjects.map((sub, idx) => (
            <span key={idx} className="text-xs font-semibold text-gray-700 bg-[#DCDCDC] px-3 py-1.5 rounded-full">{sub}</span>
          ))}
        </div>
        <div className="flex items-center gap-6 text-sm text-gray-500 mb-6 font-medium">
          <div className="flex items-center gap-2"><Calendar size={18}/> {tutor.sessions} sessions</div>
          <div className="flex items-center gap-2"><Bookmark size={18}/> Available</div>
        </div>
      </div>
      <button onClick={onRegister} className="w-full bg-[#0056b3] hover:bg-blue-700 text-white font-bold py-3 rounded-lg transition-colors">Register Tutor</button>
    </div>
  );
};
